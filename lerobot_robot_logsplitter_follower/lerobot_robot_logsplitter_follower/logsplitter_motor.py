...
class LogsplitterMotor():
    name = "logsplitter_motor"
    COM_TIMEOUT = 0.1
    ...
    def get_observation(self):
        self.send_com("READ")
        arduino_response = float(self.listen_com())
        return { "logsplitter.vel": arduino_response }
    
    @check_if_not_connected
    def send_action(self, action: RobotAction) -> RobotAction:
        self.send_com(action["logsplitter.vel"])
        arduino_response = float(self.listen_com())
        return { "logsplitter.vel": arduino_response }

    @check_if_not_connected
    def disconnect(self):
        self.send_com(0) # stop the splitter motor
        self.serial.close()
        self._connected = False
